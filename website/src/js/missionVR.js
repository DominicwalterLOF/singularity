...
function init() {
    ...
    composer1 = new EffectComposer(renderer1, renderTarget);
    composer1.setSize(window.innerWidth, window.innerHeight);
    composer1.addPass(new RenderPass(scene, camera));
    document.getElementById("mainRenderPort1").appendChild(renderer.domElement);
    document.getElementById("mainRenderPort2").appendChild(renderer1.domElement);
    loadRocket();
...
function Bloom() {
    ...
    composer1.addPass(outLine);
    composer1.addPass(halfTone);
